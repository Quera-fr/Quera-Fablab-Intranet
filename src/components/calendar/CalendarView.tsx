...
<body>
  ...
  const handlePrintAttendanceSheet = () => {
      ...
      if (s.participants) {
        s.participants.forEach((p) => {
          allParticipants.push({
            ...
            sessionType: s.type,
            sessionTitle:
              s.title || ROW_LABELS[s.type as keyof typeof ROW_LABELS],
          });
        });
      }
    });
    ...
    const deduped = {
      beneficiary: Array.from(
        new Map(
          grouped.beneficiary.map((p) => [`${p.firstname} ${p.lastname}`, p]),
        ).values(),
      ),
      volunteer: Array.from(
        new Map(
          grouped.volunteer.map((p) => [`${p.firstname} ${p.lastname}`, p]),
        ).values(),
      ),
      civic_service: Array.from(
        new Map(
          grouped.civic_service.map((p) => [`${p.firstname} ${p.lastname}`, p]),
        ).values(),
      ),
    };
          ...
          {(user.role === "admin" || user.role === "civic_service") && (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 bg-zinc-500 dark:bg-zinc-600 text-white px-4 py-2 rounded-xl hover:bg-zinc-600 dark:hover:bg-zinc-500 transition-all font-bold text-xs uppercase tracking-widest shadow-lg"
              >
              ...
              {showPrintMenu && (
                <div className="absolute top-full left-0 mt-1 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-lg z-50 min-w-max">
                  <button
                    ...
                    className="block w-full text-left px-4 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors text-sm font-semibold text-zinc-900 dark:text-zinc-100 first:rounded-t-lg"
                  >
                  ...
                  <button
                    ...
                    className="block w-full text-left px-4 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors text-sm font-semibold text-zinc-900 dark:text-zinc-100 last:rounded-b-lg border-t border-zinc-200 dark:border-zinc-700"
                  >
                    Fiche de présence
                  </button>
                </div>
