...
import type { QueraPointManager, Session, User } from "../../types";
...
<body>
    ...
    <div class="grid">
        ${dayNames
          .map((dayName, index) => {
            const sessionsDuJour = weekSessions.filter(
              (s) => new Date(s.start_time).getDay() === index + 1,
            );
            ...
            const uniqueBeneficiaires = Array.from(
              new Map(
                beneficiairesDuJour.map((p) => [
  ...
  const handleUnregister = async (sessionId: number, userId?: number) => {
    if (!user || isGuest) return;
    ...
    await fetch("/api/registrations", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ session_id: sessionId, user_id: targetUserId }),
    });
    fetchSessions();
  };
  ...
  const canDragSessions =
    !isGuest && (user?.role === "admin" || user?.role === "civic_service");
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 shadow-sm">
            <button
              type="button"
              onClick={() => {
            ...
            <span className="px-3 font-bold text-[10px] uppercase tracking-widest text-zinc-500">
              {viewMode === "week" &&
        ...
        <div className="flex gap-2">
          {isGuest ? (
            <button
              ...
              className="flex items-center gap-2 bg-black text-white px-3 py-1.5 rounded-lg hover:bg-zinc-800 transition-all font-bold text-[10px] uppercase tracking-widest shadow-lg"
            >
              Se connecter
            </button>
          ) : (
              ...
              {(user?.role === "admin" || user?.role === "civic_service") && (
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 bg-zinc-500 dark:bg-zinc-600 text-white px-3 py-1.5 rounded-lg hover:bg-zinc-600 dark:hover:bg-zinc-500 transition-all font-bold text-[10px] uppercase tracking-widest shadow-lg border border-black/20 "
                  >
                ...
                <>
                  ...
                  {isSelectionMode ? (
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 border border-zinc-200 dark:border-zinc-700 px-3 py-1.5 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-all font-bold text-[10px] uppercase tracking-wider"
                    >
            ...
            <div className="hidden md:block overflow-x-auto pt-3">
              <div className="grid gap-2 mb-2 grid-cols-7 w-full">
                {daysInWeek.map((day) => {
                  ...
                  const isCivic = !isGuest && user?.role === "civic_service";
                    ...
                    <div className="flex flex-col gap-1">
                      {daySessions.slice(0, 3).map((s) => (
                        <div
                          ...
                          className={`text-[9px] font-bold p-1 rounded whitespace-nowrap overflow-hidden text-ellipsis ${isGuest ? "cursor-default" : "cursor-pointer"} ...`}
                        >
                          {s.type === "homework_help"
                            ? "Aide devoirs"
                            : s.type === "room_booking"
      ...
      <AnimatePresence>
        ...
        {!isGuest && managerModalDate && user?.role === "admin" && (
        ...
        {!isGuest && pointsModalDate && user?.role === "civic_service" && user && (
          <QueraPointsModal
            date={pointsModalDate}
            managerUserId={user.id}
            presentBeneficiaries={Array.from(
              presentBeneficiariesByDateKey
                .get(toDateKey(pointsModalDate))
                ?.values() ?? [],
            )}
