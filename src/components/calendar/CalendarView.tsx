...
const CalendarView = ({ user }: CalendarViewProps) => {
    ...
    const ROW_LABELS: Record<string, string> = { homework_help: 'Aide aux devoirs', activity: 'Activités', room_booking: 'Réservation du local' };
    const ROW_STYLES: Record<string, string> = {
        homework_help: 'bg-blue-50/50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-900/30',
        activity: 'bg-green-50/50 dark:bg-green-900/10 border-green-100 dark:border-green-900/30',
        room_booking: 'bg-amber-50/50 dark:bg-amber-900/10 border-amber-100 dark:border-amber-900/30'
    };
...
<body>
    ...
    const handleRegister = async (sessionId: number) => {
        const res = await fetch('/api/registrations', {
            ...
            body: JSON.stringify({
                ...
                user_id: user.id,
                role_at_registration: (user.role === 'volunteer' || user.role === 'adherent') ? 'volunteer' : 'beneficiary'
            }),
        });
    ...
    const handleUnregister = async (sessionId: number, userId: number = user.id) => {
    ...
    const handleMoveSessionToDate = async (session: Session, targetDate: Date) => {
        ...
        const newStart = new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate(), start.getHours(), start.getMinutes());
        const newEnd = new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate(), end.getHours(), end.getMinutes());
    ...
    const canDragSessions = user.role === 'admin' || user.role === 'civic_service';

    const canManageRow = (rowType: string) => {
        if (user.role === 'admin') return true;
        if (user.role === 'civic_service') return rowType === 'homework_help' || rowType === 'activity';
    ...
    const calculateDefaultTimes = (defaultHour = 16, defaultMinute = 30) => {
        ...
        end.setHours(defaultHour + (defaultMinute === 30 ? 3 : 2), defaultMinute === 30 ? 30 : 0, 0, 0);
        return {
            start_time: new Date(today.getTime() - today.getTimezoneOffset() * 60000).toISOString().slice(0, 16),
            end_time: new Date(end.getTime() - end.getTimezoneOffset() * 60000).toISOString().slice(0, 16),
            date: today.toISOString().split('T')[0]
        };
    };

    const handleValidateActivity = async (activityId: number, status: 'approved' | 'pending') => {
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="flex items-center gap-4">
                    <h2 className="text-xl font-black text-zinc-900 dark:text-white uppercase tracking-tight">Planning</h2>
                    ...
                    <div className="flex items-center bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-1 shadow-sm">
                        ...
                        <span className="px-4 font-bold text-xs uppercase tracking-widest text-zinc-500">
                            {viewMode === 'week' && daysInWeek[0].toLocaleDateString('fr-FR', { month: 'short', year: 'numeric' })}
                ...
                <div className="flex gap-2">
                    ...
                    {(user.role === 'admin' || user.role === 'civic_service') && (
                        <>
                            <button onClick={handleBatchHomework} className="flex items-center gap-2 bg-blue-100 dark:bg-blue-900/40 text-blue-900 dark:text-blue-100 border border-blue-200 dark:border-blue-800 px-4 py-2 rounded-xl hover:bg-blue-200 dark:hover:bg-blue-800/60 transition-all font-bold text-xs uppercase tracking-widest shadow-lg">
                                <Plus size={16} /> Semaine Type
                            </button>
                            <button onClick={() => setShowAddRoom(true)} className="flex items-center gap-2 bg-amber-100 dark:bg-amber-900/40 text-amber-900 dark:text-amber-100 border border-amber-200 dark:border-amber-800 px-4 py-2 rounded-xl hover:bg-amber-200 dark:hover:bg-amber-800/60 transition-all font-bold text-xs uppercase tracking-widest shadow-lg">
                            ...
                            {isSelectionMode ? (
                                <>
                                    <button
                                        onClick={async () => {
                                            if (selectedSessionIds.length > 0 && confirm(`Supprimer ${selectedSessionIds.length} sessions ?`)) {
                                                await Promise.all(selectedSessionIds.map(id => fetch(`/api/sessions/${id}`, { method: 'DELETE' })));
                                    ...
                                    <button onClick={() => { setIsSelectionMode(false); setSelectedSessionIds([]); }} className="flex items-center gap-2 bg-zinc-200 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 px-4 py-2 rounded-xl hover:bg-zinc-300 dark:hover:bg-zinc-700 transition-all font-bold text-xs uppercase tracking-wider">
                                        Annuler
                                    </button>
                                </>
                            ) : (
                                <button onClick={() => setIsSelectionMode(true)} className="flex items-center gap-2 bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 border border-zinc-200 dark:border-zinc-700 px-4 py-2 rounded-xl hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-all font-bold text-xs uppercase tracking-wider">
                        ...
                        <div className="overflow-x-auto">
                            <div className="grid gap-2 mb-2 grid-cols-7 w-full">
                                {daysInWeek.map((day) => (
                                    <div
                                        key={day.toISOString()}
                                        className={`text-center p-3 rounded-2xl border shrink-0 ${day.toDateString() === new Date().toDateString()
                                    ...
                                    >
                                        {daysInWeek.map((day, i) => {
                                            const daySessions = sessions
                                                .filter(s => new Date(s.start_time).toDateString() === day.toDateString() && s.type === rowType)
                                                .sort((a, b) => a.start_time.localeCompare(b.start_time));
                                            return (
                                                <div
                                                    key={`${rowType}-${i}`}
                                                    className={`group/slot min-h-[120px] flex flex-col gap-1.5 p-2 rounded-lg bg-white/50 dark:bg-black/10 transition-all ${dragOverTarget?.rowType === rowType && dragOverTarget?.dayIndex === i && draggedSessionRef.current?.type === rowType
                                                        ? 'ring-2 ring-black dark:ring-white ring-dashed ring-offset-2'
                                                        : ''
                                                        }`}
                                                    onDragOver={(e) => {
                                                        if (canDragSessions && draggedSessionRef.current?.type === rowType) {
                                                            const isOccupied = daySessions.length > 0 && daySessions[0].id !== draggedSessionRef.current?.id;
                                                            if (!isOccupied) {
                                                                e.preventDefault();
                                                    ...
                                                    onDrop={(e) => {
                                                        ...
                                                        if (!canDragSessions || !draggedSessionRef.current) return;
                                                ...
                                                >
                                                    {daySessions.map(s => {
                                                        const isRegistered = s.participants.some(p => p.user_id === user.id);
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                ...
                                                                {isRegistered && <div className="absolute top-2 right-2 text-black dark:text-white"><CheckCircle size={14} /></div>}
                                                            </motion.div>
                                                        );
                                                    })}
                                                    {canManageRow(rowType) && daySessions.length === 0 && (
                                                        <button
                    ...
                    {viewMode === 'month' && (
                        <div className="grid grid-cols-7 gap-2">
                            {['Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam', 'Dim'].map((d, i) => (
                                <div key={i} className="text-center text-[10px] font-black uppercase tracking-widest text-zinc-400 py-2">{d}</div>
                            ))}
                            {daysInMonth.map((day, i) => {
                                ...
                                return (
                                    <div key={i} className={`min-h-[100px] border rounded-xl p-2 ${isCurrentMonth ? 'bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-700 text-zinc-900 dark:text-white' : 'bg-zinc-50 dark:bg-zinc-950 border-zinc-100 dark:border-zinc-800 text-zinc-400'} ${isToday ? 'ring-2 ring-black dark:ring-white' : ''}`}>
                                        ...
                                        <div className="flex flex-col gap-1">
                                            ...
                                            {daySessions.length > 3 && <div className="text-[8px] font-black text-zinc-400 text-center uppercase">+{daySessions.length - 3} autres</div>}
                                        </div>
                                    </div>
                                );
                            })}
                    ...
                    {viewMode === 'year' && (
                        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {monthsInYear.map((monthDate, i) => (
                                <div key={i} className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-4">
                                    <h4 className="text-sm font-black uppercase tracking-widest mb-4 hover:underline cursor-pointer" onClick={() => { setCurrentDate(monthDate); setViewMode('month'); }}>
                                        {monthDate.toLocaleDateString('fr-FR', { month: 'long' })}
                                    </h4>
                                    <div className="grid grid-cols-7 gap-1">
                                        {Array.from({ length: new Date(monthDate.getFullYear(), monthDate.getMonth() + 1, 0).getDate() }, (_, d) => {
                                            const count = sessions.filter(s => new Date(s.start_time).toDateString() === new Date(monthDate.getFullYear(), monthDate.getMonth(), d + 1).toDateString()).length;
            ...
            <AnimatePresence>
                ...
                {showAddActivity && <ActivityFormModal user={user} calculateDefaultTimes={calculateDefaultTimes} onClose={() => setShowAddActivity(false)} onFetchSessions={fetchSessions} />}
                {showAddHomework && <HomeworkFormModal calculateDefaultTimes={calculateDefaultTimes} onClose={() => setShowAddHomework(false)} onFetchSessions={fetchSessions} />}
                {showAddRoom && <RoomBookingFormModal user={user} calculateDefaultTimes={calculateDefaultTimes} onClose={() => setShowAddRoom(false)} onFetchSessions={fetchSessions} />}
            </AnimatePresence>

            {successMessage && <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-black text-white px-8 py-4 rounded-3xl font-black uppercase tracking-widest text-xs shadow-2xl z-[60] animate-in fade-in slide-in-from-bottom-8">{successMessage}</div>}
        </div>
    );
};
