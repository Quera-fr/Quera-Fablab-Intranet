...
test.describe("SessionModal - Administrateur", () => {
    ...
    test("admin inscrit manuellement un bénéficiaire (POST intercept)", async ({
        ...
        const now = new Date();
        const mockSelectableBeneficiary = {
            id: 4242,
            email: "bene@test.fr",
            lastname: "Martin",
            firstname: "Alice",
        ...
        const session = {
            ...
            title: "Session bénéficiaire test",
            status: "approved",
            participants: [],
        };
        ...
        await page.route("**/api/users", async (route) => {
            await route.fulfill({
                status: 200,
                contentType: "application/json",
                body: JSON.stringify([mockSelectableBeneficiary]),
            });
        });

        await loginAsAdmin(page);

        await expect(page.locator("text=Session bénéficiaire test")).toBeVisible({
            timeout: 10000,
        });

        await page.click("text=Session bénéficiaire test");
        await expect(page.locator("div.fixed.inset-0 h3")).toBeVisible();
        ...
        const beneficiaryOption = page.locator('[class*="rs__option"]', {
            hasText: "Alice Martin",
        });
        await expect(beneficiaryOption).toBeVisible({ timeout: 5000 });
        ...
        expect(postedBody.user_id).toBe(4242);
        expect(postedBody.role_at_registration).toBe("beneficiary");
    });
    ...
    test("admin inscrit manuellement un bénévole", async ({ page }) => {
        ...
        await page.route("**/api/users", async (route) => {
            await route.fulfill({
                status: 200,
                contentType: "application/json",
                body: JSON.stringify([mockVolunteer]),
            });
        });

        await loginAsAdmin(page);

        await expect(page.locator("text=Session bénévole test")).toBeVisible({
            timeout: 10000,
        });

        await page.click("text=Session bénévole test");
        await expect(page.locator("div.fixed.inset-0 h3")).toBeVisible();
        ...
        const volunteerControl = page.locator('[class*="rs__control"]').nth(1);
        await volunteerControl.click();

        const volunteerOption = page.locator('[class*="rs__option"]', {
            hasText: "Victor Volunteer",
        });
        await expect(volunteerOption).toBeVisible({ timeout: 5000 });
        ...
        expect(postedBody.user_id).toBe(8888);
        expect(postedBody.role_at_registration).toBe("volunteer");
    });
