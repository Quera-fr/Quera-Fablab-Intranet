...
test.describe("Calendar E2E Tests", () => {
	...
	test.describe("1. Navigation & View Modes", () => {
		test("Week View (default) displays correctly", async ({ page }) => {
			...
			await expect(
				page.locator("text=Réservation du local").first(),
			).toBeVisible();
			// Check view buttons
			await expect(
		...
		test("Month View toggles and renders grid", async ({ page }) => {
			...
			await expect(
				page.getByText("Lun", { exact: true }).first(),
			).toBeVisible();
			await expect(
				page.getByText("Dim", { exact: true }).first(),
			).toBeVisible();
			// 1st of month should exist
			await expect(
				page.locator("p.text-xs.font-black", { hasText: /^1$/ }).first(),
			).toBeVisible();
		});
		...
		test("Time Navigation works in week view", async ({ page }) => {
			const dateTextLocator = page.locator("span.font-bold");
	...
	test.describe("2. Empty Slots & Quick Create", () => {
		...
		test("Clicking empty Aide aux devoirs slot opens modal", async ({
			page,
		}) => {
			// The empty slot create button (+ icon). The row class uses opacity changes on hover.
			const addBtn = page
				.locator("button.group-hover\\/slot\\:opacity-100")
				.first();
			...
			await expect(
				page.locator("h3", { hasText: "Nouvelle Permanence" }),
			).toBeVisible();
	...
	test.describe("3. Main Creation Modals (Header Buttons)", () => {
		test("Create Activity Flow", async ({ page }) => {
			...
			await page
				.getByRole("button", { name: "Nouvelle Activité", exact: true })
				.click();
			await expect(
				page.locator("h3", { hasText: "Nouvel Atelier" }),
			).toBeVisible();
		...
		test("Create Room Booking Flow", async ({ page }) => {
			...
			await page
				.getByRole("button", { name: "Résa. Local", exact: true })
				.click();
			await expect(
				page.locator("h3", { hasText: "Réservation du local" }).last(),
			).toBeVisible();

			await Promise.all([
				page.waitForResponse(
					(response) =>
						response.url().includes("/api/sessions/room") &&
						response.request().method() === "POST",
				),
	...
	test.describe("5. Advanced Scheduling Tools", () => {
		test("Multi-selection Deletion (Sél. Multiple)", async ({ page }) => {
			...
			const sessions = page.locator(
				"div.cursor-pointer.relative.group.overflow-hidden",
			);
		...
		test("Drag and Drop Session", async ({ page }) => {
			...
			if (
				(await sourceSession.isVisible()) &&
				(await dropTargets.count()) > 1
			) {
